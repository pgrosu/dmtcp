#ifndef __STATIC_DMTCP_H_
#define __STATIC_DMTCP_H_
...
#define DELETE_CALL_FRAME() \
  ({asm volatile ("mov %%rax, %%r10\n\t" : : : );\
    ...
    asm ("pop %%rbp\n\t"\
         ...
         :\
         : ); })
EXTERNC int __clone(int (*fn)(void *arg),
                    void *child_stack,
                    int flags,
                    void *arg,
                    int *ptid,
...
({\
  ...
  if (_real_ ## func == (__typeof__(&func)) -1) {\
     _real_ ## func = (__typeof__(&func)) dmtcp_sdlsym( # func, \
                                                       (void *)&func, \
                                                       STATIC_PLUGIN_ID, \
                                                       func ## addrs);\
  }\
